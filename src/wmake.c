...
int main(int arc, char * argv[])
{
    ...
    if (update(ASSEMBLE_FILES, 5, "assemble.exe",
        "g++ ../src/config.c ../src/commands.c ../src/assemble.c -o assemble.exe"))
    {
        printf("Make aborted!\n");
        return 0;
    }

    if (update(DISASSEMBLE_FILES, 7, "disassemble.exe",
        "g++ ../src/config.c ../src/commands.c ../src/code_utils.c ../src/disassemble.c -o disassemble.exe"))
    {
    printf("Make aborted!\n");
    return 0;
    }

    if (update(RUN_FILES, 9, "run.exe",
        "g++ ../src/config.c ../src/commands.c ../src/code_utils.c ../../MyStack/stack.c ../src/run.c -o run.exe"))
    {
    printf("Make aborted!\n");
    return 0;
    }
    ...
    if (arc >= 2)
    {
        char asm_command[50] = "assemble.exe ";
        strcat(asm_command, argv[1]);
        strcat(asm_command, " ");
        strcat(asm_command, BYTECODE_FILE);
        system(asm_command);

        char run_command[50] = "run.exe ";
